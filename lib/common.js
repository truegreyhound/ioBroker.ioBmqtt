...
function state2string(iobState, bSendStateObject, bioBMFActive, nioBMFCompressFromLength, sClientId, receiver, sCoding, that) {
    ...
    try {
        ...
        if (sCoding == 'base64' && (nioBMFCompressFromLength == 0 || (nioBMFCompressFromLength > 0 && msg.length <= nioBMFCompressFromLength))) msg = base64.encode(msg);

        if (nioBMFCompressFromLength > 0 && msg.length > nioBMFCompressFromLength) {
            msg = zip(msg);

            sCoding_src = (sCoding == 'base64') ? 'object' : 'size';
            sCoding = 'zip';
        }
