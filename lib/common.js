...
function state2string(val, sendStateObject, ioBrokerMessageFormatActive, ioBrokerMessageFormatCompressFromLength, clientId, receiver, coding, type) {
    let msg;
    let coding_src = '';
...
//!P!            msg = JSON.stringify({"clientID": clientId, "ts": (new Date()).getTime(), "receiver": ((typeof receiver == 'string') ? receiver : JSON.stringify(receiver)), "message": msg});
            msg = JSON.stringify({"clientID": clientId, "ts": (new Date()).getTime(), "version": ioBrokerMessageFormatVersion, "type": type, "coding": coding, "coding-src": coding_src, "receiver": JSON.stringify(receiver), "message": msg});
        }
