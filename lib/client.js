...
function MQTTClient(adapter, states) {
    ...
    function publishAllStates(config, toPublish) {
        ...
        if (adapter.config.extraSet && states[id] && !states[id].ack) {
            ...
            if (states[id]) {
                adapter.log.debug('client.publish, topic: ' + id2topic[id] + '; message: ' + state2string(states[id], adapter.config.sendStateObject) + '; qos: ' + adapter.config.defaultQoS + '; retain: ' + adapter.config.retain);
    ...
    (function _constructor(config) {
        ...
        client.on('message', (topic, message) => {
            ...
            if (config.debug) {
                adapter.log.debug('Broker publishes "' + topic + '": ' + message);
            }
                ...
                adapter.getObject(id, (err, obj) => {
                    if (!obj) {
                        ...
                        if (namespaceRegEx.test(obj._id) && (!obj.native || !obj.native.topic)) {
                            ...
                            config.debug && adapter.log.debug('Object updated, id: ' + obj._id + '; old_obj: ' + JSON.stringify(old_obj) + '; obj: ' + JSON.stringify(obj));
                        }
