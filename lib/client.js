...
/*!P!        publishMessage(
        ...
        if (config.patterns.length > 0 && (typeof config.patterns !== 'string') && config.patterns.toString() !== '') {
            adapter.config.patterns.forEach((item) => {
                ...
                */
                    ...
                    adapter.log.info(fctName + ' invalid pattern: "' + JSON.stringify(item) + '"');
                }
            });
    ...
    function publishMessage(id, topic, message, coding) {
        ...
        let msg = state2string(message, adapter.config.sendStateObject, adapter.config.ioBrokerMessageFormatActive, adapter.config.ioBrokerMessageFormatCompressFromLength, adapter.config.clientId, '*', coding);
...
*/
        ...
        if (topic && topic !== '' && (!config.prefix || config.prefix === '' || topic !== config.prefix)) {
            ...
            adapter.log.debug(fctName + ', no topic?; id: ' + id + '; topic: "' + topic + '"');
            ...
            adapter.getForeignObject(id, (err, obj) => {
                ...
                if (obj.native && obj.native.mqtt_coding) {
                    coding = obj.native.mqtt_coding;
                }
                adapter.log.debug(fctName + ', getForeignObject; topic: ' + topic + '; coding: "' + coding + '"');
        ...
        if (!coding) {
            adapter.getForeignObject(id, (err, obj) => {
                ...
                adapter.log.debug(fctName + ', getForeignObject, determine coding: ' + JSON.stringify(obj) + '; err: ' + JSON.stringify(err));
                ...
                if (!obj) {
                    ...
                    return;
                } else if (obj.native && obj.native.mqtt_coding) {
                    coding = obj.native.mqtt_coding;
                }
        ...
        let msg = state2string(states[id].state, config.sendStateObject, config.ioBrokerMessageFormatActive, config.ioBrokerMessageFormatCompressFromLength, config.clientId, '*', coding);
    ...
    (function _constructor(config) {
        ...
        */
        ...
        client.on('message', (topic, message) => {
            ...
            if (adapter.config.debug) adapter.log.debug(fctName + ' received, topic: "' + topic + '", message: ' + message);
