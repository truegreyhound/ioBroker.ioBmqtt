...
function MQTTClient(adapter, states) {
    ...
    this.onMessage = (topic, message) => publishMessage(id, topic, message, coding);    // publish to broker
    ...
    function send2Broker(id, state, cb) {
        ...
        adapter.log.info(fctName + ' started for ' + id + ' [' + topic + ']; state "' + JSON.stringify(state) + '"');
...
/*!P!        publishMessage(
        ...
        if (config.patterns.length > 0 && (typeof config.patterns !== 'string') && config.patterns.toString() !== '') {
            adapter.config.publish.forEach((item) => {
                if ((item) && item.enabled && item.pattern && item.pattern.trim() != '') {
                     try {
                        let qos = (item.QoS && (parseInt(item.QoS) >= 0  && parseInt(item.QoS) <=2) ? parseInt(item.QoS) : config.defaultQoS_publish);

                        adapter.log.info(fctName + ', topic: "' + item.pattern.trim() + '"; QoS: "' + qos + '"; enabled: "' + item.enabled + '"');
    ...
    function publishMessage(id, topic, message, coding) {
        ...
        let qos = adapter.config.defaultQoS_publish;
        let retain = adapter.config.retain;

        if (id && id != '') {
            qos = (states[id] && states[id].qos && (parseInt(states[id]) >= 0  && parseInt(states[id]) <=2) ? parseInt(states[id]) : adapter.config.defaultQoS_publish);
        ...
        adapter.log.info('> client.publish(' + topic + (adapter.config.extraSet ? '/set' : '') + ', ' + msg + ', {"qos": ' + qos + ', "retain": ' + adapter.config.retain + '})');

        client.publish(topic + (adapter.config.extraSet ? '/set' : ''), msg, {qos: qos, retain: retain});
    }
...
*/
        ...
        config.debug && adapter.log.info('> client.publish(' + topic + (config.extraSet ? '/set' : '') + ', ' + msg + ', {"qos": ' + states[id].qos + ', "retain": ' + states[id].retain + '})');

        client.publish(topic + (config.extraSet ? '/set' : ''), msg, {qos: states[id].qos, retain: states[id].retain}, err => {
...
*/
    ...
    (function _constructor(config) {
        ...
        adapter.log.debug('Try to connect to broker with ' + JSON.stringify({
            keepalive: config.keepalive_s || 10, /* in seconds */
            protocolId: 'MQTT',
            protocolVersion: 4,
        ...
        try {
            ...
            adapter.log.error('Can not connect to mqtt broker: ' + err);
            typeof adapter.terminate === 'function' ? adapter.terminate(11) : process.exit(11);

            return;
        }

/*!P!        if (typeof config.patterns === 'string') {
