...
//!P!    const id2topic  = {};
    ...
    // declare events
//!P!    this.onStateChange = (id, state) => send2Server(id, state);             // change state on ioBroker
    this.onStateChange = (id, state) => send2Broker(id, state);             // change state on ioBroker
    ...
    function subscribeConfiguredTopics(config) {
        ...
        if (config.patterns.length > 0 && (typeof config.patterns !== 'string')) {
            adapter.log.debug(fctName + ', start subscribe configured pattern ...');
            ...
            adapter.log.info(fctName + ' start subscribe configured pattern - no pattern configured., ');
        }
    ...
    function publishMessage(topic, message) {
        if (!message) {
            message = null;
            adapter.log.debug('Send to broker "' + adapter.config.prefix + topic + '": deleted');
        } else {
            adapter.log.debug('Send to broker "' + adapter.config.prefix + topic + '": ' + JSON.stringify(message));
        }

        adapter.log.debug('> client.publish(' + adapter.config.prefix + topic + ', ' + state2string(message, adapter.config.sendStateObject) + ', {qos: ' + adapter.config.defaultQoS + ', retain: ' + adapter.config.retain + '})');

        client.publish(adapter.config.prefix + topic, state2string(message, adapter.config.sendStateObject), {qos: adapter.config.defaultQoS, retain: adapter.config.retain});
    }
    ...
    function publishAllStates(config, toPublish) {
        ...
        adapter.log.info('client.publish topic: ' + (adapter.config.extraSet ? '/set' : '') + topic + '; message: ' + state2string(states[id], adapter.config.sendStateObject) + '; qos: ' + adapter.config.defaultQoS + '; retain: ' + adapter.config.retain);

        client.publish((adapter.config.extraSet ? '/set' : '') + topic, state2string(states[id], adapter.config.sendStateObject), {qos: adapter.config.defaultQoS, retain: adapter.config.retain}, err => {
    ...
    (function _constructor(config) {
        ...
        */
        function writeMessage2State(iobObj, sObjType, id, message, config) {
            ...
            adapter.log.debug('writeMessage2State,  iobObj: ' + JSON.stringify(iobObj) + ', sObjType: ' + sObjType + ', id: ' + id + ' message (' + typeof message + '): ' +  JSON.stringify(message));

            adapter.getForeignState(id, function (err, oldState) {
                adapter.log.debug('writeMessage2State (known ' + sObjType + ' obj) getForeignState for "' + id + '": ' + JSON.stringify(oldState));
                ...
                if (oldState) {
                    ...
                    if (typeof message === 'object') {
                        //!P!if (!config.save_onchange || JSON.stringify(oldState) !== JSON.stringify(message) || oldState.ack !== isAck) {
                        if (!config.save_onchange || oldState.val !== message.val || oldState.ack !== message.ack) {
                            adapter.setForeignState(id, message);
    
                            config.debug && adapter.log.info('writeMessage2State (new ' + sObjType + ' obj), set state object "' + id + '": ' + JSON.stringify(message));
                        } else {
                            config.debug && adapter.log.info('writeMessage2State (known ' + sObjType + ' obj) for "' + id + '" no updated needed for state object ' + JSON.stringify(message));
                        }
                    } else {    // value
                        if (!config.save_onchange || oldState.val !== message || oldState.ack !== isAck) {

                            adapter.setForeignState(id, {val: message, ack: isAck});

                            config.debug && adapter.log.info('writeMessage2State (known ' + sObjType + ' obj), set state "' + id + '": ' + JSON.stringify({val: message, ack: isAck}));
                        } else {
                            config.debug && adapter.log.info('writeMessage2State (known ' + sObjType + ' obj) for "' + id + '" no updated needed for state "' + message + '"');
                        }
                    }
                } else {
                    ...
                    if (typeof message === 'object') {
                        ...
                        config.debug && adapter.log.info('writeMessage2State (new ' + sObjType + ' obj), set state "' + id + '": ' + JSON.stringify({val: message, ack: isAck}));
                    }
                }
            });
        } // writeMessage2State()
        ...
        client.on('message', (topic, message) => {
            ...
            config.debug && adapter.log.debug('client.on received from broker "' + topic + '": ' + message.toString());
            ...
            let id = convertTopic2id(topic, false, config.prefix, adapter.namespace);
            ...
            config.debug && adapter.log.debug('client.on message received, topic: "' + topic + '", message: ' + message);
            ...
            //!D!config.debug && adapter.log.debug('client.on message received state "' + topic + '"');
            ...
            adapter.getObject(id, (err, obj) => {
                if (obj) {
                    ...
                    if (namespaceRegEx.test(obj._id) && (!obj.native || !obj.native.topic)) {
                        ...
                        adapter.log.debug('Object updated, id: ' + obj._id + '; old_obj: ' + JSON.stringify(old_obj) + '; obj: ' + JSON.stringify(obj));
                    }
                    ...
                    adapter.getForeignObject(id, (err, obj) => {
                        if (obj) {
                            ...
                            if (aID.length > 2) {
                                adapter.getForeignObject(aID[0] + '.' + aID[1] + '.' + aID[2], (err, obj) => {
                                    if (obj) {
                                    ...
                                    adapter.log.debug('Object created, id: ' + id + '; iobObj: ' + JSON.stringify(iobObj));
            ...
            if (config.publishAllOnStart) {
                ...
                Object.keys(states).forEach(id => {
                    ...
                    if (!messageboxRegex.test(id)) {
                        ...
                        adapter.log.debug('Test states for publishing: "' + id + '", error in regex');
                    }
                });
